...
def _validate_loopback_host(host: str) -> None:
    """Validate that the host is loopback-only for Local Web."""
    ...
    if not ip.is_loopback:
        raise ValueError(
            ...
            f"Local Web only allows loopback addresses."
        )
...
def _check_runtime_config() -> None:
    """Check critical runtime configuration before serving Local Web."""
    checks: list[str] = []
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        get_tokenizer()
        reconciled = await start_document_processing_runtime(session_factory)
...
async def enforce_loopback_origin(request: Request, call_next):
    """Enforce loopback-only origin for HTTP requests."""
    if not settings.is_development:
        ...
        if not origin and referer and not _is_loopback_origin(referer):
