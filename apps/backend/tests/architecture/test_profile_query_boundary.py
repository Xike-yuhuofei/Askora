...
def test_users_api_no_longer_imports_legacy_profile_orm() -> None:
    ...
    assert not any(
        violation.rule == "API_NO_LEARNER_PERSISTENCE" and violation.module == "app.models.profile"
...
@pytest.mark.parametrize(
    ("module", "statement"),
    [
        ("app.models.profile", "from app.models.profile import UserProfile"),
        (
        ...
        (
            ...
            "from app.infrastructure.learning_records import LearningRecordRepository",
        ),
    ],
)
def test_api_direct_learner_persistence_read_is_blocked(
    tmp_path, module: str, statement: str
) -> None:
...
def test_api_no_learner_persistence_allowance_remains() -> None:
    ...
    assert not any(item.rule == "API_NO_LEARNER_PERSISTENCE" for item in LEGACY_ALLOWLIST)
