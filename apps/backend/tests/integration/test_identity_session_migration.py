"""EXEC034 migration upgrade, rollback and forward-fix evidence."""
...
@pytest.mark.asyncio
async def test_identity_session_migration_upgrade_rollback_forward_fix(tmp_path: Path) -> None:
    database_url = f"sqlite+aiosqlite:///{tmp_path / 'identity-migration.db'}"
    ...
    async with engine.connect() as connection:
        ...
        assert {"auth_sessions", "identity_command_receipts"}.issubset(tables)
        assert {"credential_version", "password_changed_at"}.issubset(user_columns)
    ...
    async with rolled_back.connect() as connection:
        ...
        assert "credential_version" not in user_columns
