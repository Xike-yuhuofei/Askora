...
export default function Evidence() {
  ...
  return (
    <div className="product-view page-stack">
      ...
      <section className="surface" aria-labelledby="evidence-items-title">
        <div className="section-heading"><div><p className="eyebrow">逐条证据</p><h2 id="evidence-items-title">证据列表</h2></div><ShieldCheck size={20} /></div>
        {evidenceItems.length ? <ul className="evidence-list evidence-items">{evidenceItems.map((item, index) => <li key={item.evidence_id || `evidence-${index}`}><div className="evidence-list__heading"><div><h3>{evidenceTypeLabels[item.type] || item.type || '证据类型未知'}</h3><p>{item.source_activity ? `来源活动：${item.source_activity}` : '来源活动未知'}</p></div><div className="evidence-estimate"><span className="status-pill">{evidenceStatusLabels[item.status] || item.status || '状态未知'}</span><small>{formatEvidenceTime(item.occurred_at)}</small></div></div><p className="data-note">结果：{item.outcome == null || item.outcome === '' ? '暂无结果' : item.outcome}</p></li>)}</ul> : <div className="empty-panel"><ShieldCheck size={24} /><p>还没有足够的学习证据</p><p>先完成学习活动或独立验证，证据会逐步积累。</p></div>}
      </section>
