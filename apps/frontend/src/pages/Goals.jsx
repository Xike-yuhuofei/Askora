  ...
  return (
    <div className="product-view page-stack">
      <header className="page-header page-header--split">
        ...
        <div className="goal-header-actions"><span className="status-pill status-pill--available">{data.goals.length} 个目标</span><button type="button" className="button button--primary" onClick={() => navigate('/learning/goals/new')}><Plus size={16} />创建目标</button></div>
      </header>
      ...
      {data.goals.length ? (
        ...
        <section className="surface empty-panel"><Target size={24} /><h2>还没有学习目标</h2><p>从已有资料定义一个可测目标；未就绪资料仍可保存到草稿。</p><div className="goal-row-actions"><button type="button" className="button button--primary" onClick={() => navigate('/learning/goals/new')}>创建目标</button><button type="button" className="button button--secondary" onClick={() => navigate('/library')}>前往资料库</button></div></section>
      )}
