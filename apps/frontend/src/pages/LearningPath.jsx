...
function activityIdFromRef(ref) { return ref?.split(':')[1] || '' }
...
export default function LearningPath() {
  ...
  const load = async (scope = goalId) => {
    ...
    try {
      ...
      setState({ status: 'error', payload: null, goalOptions: [], error: '学习路径暂时无法读取。' })
    }
  }
  ...
  if (state.status === 'error') return <div className="page-state page-state--error" role="alert"><h1>学习路径</h1><p>{state.error}</p><button type="button" className="button button--secondary" onClick={() => load()}><RefreshCw size={16} />重试</button></div>
    ...
    <div className="product-view page-stack">
      ...
      {path ? (
        <>
          ...
          <section className="surface" aria-labelledby="activity-list-title"><div className="section-heading"><div><p className="eyebrow">规划顺序</p><h2 id="activity-list-title">学习活动</h2></div><span className="status-pill">{path.activities.length} 项</span></div>{path.activities.length ? <ol className="path-list">{path.activities.map((activity, index) => { const action = activity.launch_state === 'RESUMABLE' ? '继续学习' : activity.launch_state === 'REQUIRES_START_COMMAND' ? '开始学习' : ''; return <li key={activity.activity_ref}><span className="path-list__index">{index + 1}</span><div><div className="product-row__title"><h3>{activity.title}</h3><span className="status-pill">{activityStatus[activity.status] || activity.status}</span></div><p>{activity.estimated_duration_minutes} 分钟 · {activityTypes[activity.type] || '学习活动'}</p><small>{activity.reason_codes.map((reason) => reasonLabels[reason] || '由当前学习计划安排').join(' · ')}</small>{action && <button type="button" className="button button--secondary" onClick={() => navigate(`/learn/${encodeURIComponent(activityIdFromRef(activity.activity_ref))}`)}>{action}<ArrowRight size={16} /></button>}</div></li> })}</ol> : <p className="empty-copy">计划存在，但当前没有可展示的活动。</p>}</section>
        </>
      ) : !data.reason_codes.includes('MULTIPLE_CURRENT_PLANS_REQUIRE_GOAL_SCOPE') && (
        <section className="surface empty-panel"><ListTree size={24} /><h2>还没有可展示的学习路径</h2><p>这不表示目标已完成；当前没有 owner 发布的活动计划。</p><button type="button" className="button button--secondary" onClick={() => navigate('/learning/goals')}>查看学习目标<ArrowRight size={16} /></button></section>
      )}
