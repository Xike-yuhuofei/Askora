...
export default function GoalDetail({ goalId }) {
  ...
  return <div className="goal-editor page-stack">
    ...
    {goalState.status === 'active' && <section className="surface"><div className="section-heading"><div><p className="eyebrow">版本化验证</p><h2>达成证据</h2></div><ShieldCheck size={20} /></div><p>评分绑定当前目标、资料、rubric 和 policy；低置信、评分分歧或模型失败只会进入复核/失败状态。</p>{!workspace?.assessments?.length && <button disabled={busy} aria-busy={busy} className="button button--primary" onClick={schedule}>安排成功标准验证</button>}<div className="goal-assessments">{workspace?.assessments?.map((item) => <article className="goal-assessment-card" key={item.assessment_activity_id}><div><strong>{item.prompt}</strong><span>{assessmentLabels[item.status] || item.status} · {item.scoring_method === 'structured' ? '确定性评分' : '开放题双重评分'}</span></div>{['available', 'needs_review', 'scoring_failed'].includes(item.status) && <><label htmlFor={`answer-${item.assessment_activity_id}`}>你的独立回答</label><textarea id={`answer-${item.assessment_activity_id}`} value={responses[item.assessment_activity_id] || ''} onChange={(event) => setResponses((current) => ({ ...current, [item.assessment_activity_id]: event.target.value }))} /><button disabled={busy || !(responses[item.assessment_activity_id] || '').trim()} aria-busy={busy} className="button button--secondary" onClick={() => submit(item)}>提交验证</button></>}</article>)}</div>{workspace?.assessments?.length > 0 && <button disabled={busy} aria-busy={busy} className="button button--secondary" onClick={evaluate}>检查达成门禁</button>}{workspace?.latest_evaluation?.eligible_for_achievement && <button disabled={busy} aria-busy={busy} className="button button--primary" onClick={achieve}>由我确认目标达成</button>}</section>}
  </div>
}
