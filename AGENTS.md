...
> 可代表该目标接受必要架构决定，但必须先完成 ADR/Spec/EXEC 治理再修改产品代码。
...
1. 继续完成不受该缺口影响的部分；
2. 输出 `SPEC GAP`、受影响规则、候选方案、推荐选择和验证路径；
3. 已获第 4 节用户委托时，由 Codex 选择推荐方案，显式创建并接受所需 ADR、
   更新 Spec、冻结 EXEC 后继续实现；
4. 未获该委托时，等待用户或顶层设计流程完成选择；
5. 无论由谁选择，都不得先改代码、再用文档追认既成事实。
