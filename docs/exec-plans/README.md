...
> Active：EXEC-023～EXEC-024
> 已完成：EXEC-001～EXEC-022
...
| 目录 | 当前状态 | 规则 |
|---|---|---|
| `active/` | EXEC-023～024 READY | 必须按依赖 gate 执行；后序 READY 不代表可跳过前序 |
| [`completed/`](completed/README.md) | EXEC-001～022 | 保留执行任务合同及其显式决策记录 |
...
| Book-to-Learning SPEC-D01～D06 | EXEC-017～024 | IN PROGRESS（EXEC-017～022 DONE） |
