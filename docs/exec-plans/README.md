...
> 当前状态：Local Single-User / no-auth 基线与 CI v2 基础切换已合并；UI-03A EXEC-043 已验收归档，其余 UI-03 acceptance 与 CI v2 chain 继续执行。  
> Active / Frozen Queue：EXEC-044～046、EXEC-054～059  
> UI-03 acceptance chain：`EXEC-043 DONE → EXEC-044 → EXEC-045 → EXEC-046`  
...
UI-03 代码存在度高于原 EXEC 生命周期记录，因此处理原则固定为：
...
EXEC-043 已按该原则完成验收并归档；EXEC-044～046 继续严格按 acceptance chain 收口。禁止因为历史文档写过 blocked 就整套重做已有正确实现。

## 2. Active / Frozen Queue

| EXEC | Task | Status | Dependency / Current handling |
|---|---|---|---|
| [EXEC-043](completed/EXEC-043-ui-03a-shell-routes-learning-domain.md) | UI-03A Shell, Routes and Learning Domain | **DONE / ARCHIVED** | 已完成 current-main conformance acceptance；作为后续 UI-03 基线 |
| [EXEC-044](active/EXEC-044-ui-03b-today-primary-hierarchy.md) | UI-03B Today Primary Hierarchy | FROZEN / READY_AFTER_EXEC_043 | main 已有实现迹象；按 frozen AC 验收/补 Gap |
...
| UI-03A Shell / Routes / Learning Domain | EXEC-043 | DONE / archived 2026-08-10 |
...
剩余 EXEC-044～046 仍必须按顺序验收。即使代码已在较早的大 PR 中提前合并，也不得跳过逐项 AC；同样也不得因为历史文档仍有旧状态措辞就重复实现已经正确的代码。
...
- UI-03 EXEC-044～046 严格按 acceptance 顺序处理；
