...
| docs lifecycle/link gate | PASS on candidate Git tree；用户未跟踪 `docs/engineering/README.md` 不属于 release candidate |
...
- 用户工作区存在未跟踪 `docs/engineering/README.md`，其 `file://` 链接会令直接在脏工作区运行 docs checker 失败；该文件未被读取为合同、未修改、未暂存，也不进入候选 Git tree。
