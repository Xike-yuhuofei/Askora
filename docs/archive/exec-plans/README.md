# Askora Archived Execution Plans

> 本目录同时保存 DONE、superseded 与历史执行报告；它是审计索引，不是实时 queue。实时状态以 Linear + current `main` 为准。
...
| EXEC-045 — UI-03C Library Progressive Disclosure | SUPERSEDED | 由 current EXEC-072 / XIK-165 替代；不得执行 |
| EXEC-060 — v1 Standalone Local Runtime Closure | DONE | Linear XIK-167 completed 2026-08-10 |
| EXEC-061 — Workspace / Project / Session Persistence & Migration | DONE | Linear XIK-171 completed 2026-08-10 |
| EXEC-062 — Workspace-scoped Learner State Projection | DONE | Linear XIK-177 completed 2026-08-10 |
| EXEC-063 — Workspace-scoped Retrieval Cutover | DONE | Linear XIK-172 completed 2026-08-10 |
| EXEC-065 — Material Trash / Restore / Permanent Delete | DONE | Linear XIK-174 completed 2026-08-11 |
| EXEC-067 — v1 Product Positioning Conformance Release Gate | DONE | Linear XIK-176 completed 2026-08-11 |
...
详细审计与遗留债务见：`docs/archive/releases/v0.2-first-vertical-learning-loop.md`。
...
详细 DoD/AC、测试、迁移、恢复、安全、OPVE 和 evidence boundary 见：`docs/archive/releases/v0.3-adaptive-teaching-loop.md`。
...
UI-01 交付 learning-loop-first Shell、Today Workspace Query、明确标记的兼容导师工作台、History 与 Settings；未实现或伪造 canonical Goal/Plan/Activity link。详细证据见 `docs/archive/releases/ui-01-learning-shell-workspace.md`。
...
UI-02A 交付 current-user Canonical 资料库、durable document processing、source-bound KnowledgeUnit candidates、范围化 Knowledge Map 与 SourceSpan Inspector；无证据时关系保持为空。详细证据见 `docs/archive/releases/ui-02a-library-knowledge-map.md`。
...
UI-02B2 交付 system-guided safe auto-advance、rank-1 primary diagnostic target、system-start 与 durable activity transcript；其当次报告记录的本地数据库状态为 pending migration。详细证据见 `docs/archive/releases/ui-02b2-guided-book-learning.md`。
...
详细证据见 `docs/archive/releases/ui-02b3-real-model-guided-learning.md`。
...
UI-02B 交付 current-user Goals、canonical plan order、owner-safe Evidence 与诚实 Today 计划摘要；未冻结的 activity/session link 不以 UI 状态伪造。详细证据见 `docs/archive/releases/ui-02b-goals-path-evidence.md`。
...
P1-03 交付 macOS 私人桌面 SQLite 的加密恢复、可验证离线恢复、current-user 可读导出和 owner-coordinated 四范围删除；详细 AC、测试、打包桌面恢复证据与既有仓库门禁债务见 `docs/archive/releases/p1-03-data-control-recovery.md`。
